...
async function readDbBlob() {
  ...
  // Walk newest → older; a just-written blob may briefly 403/404 on edge
  // propagation, in which case we accept the previous version.
  for (const blob of sorted) {
    const res = await fetch(blob.url, { cache: 'no-store' });
    if (res.ok) return { ...DEFAULT_DB, ...(await res.json()) };
